...
print("This is modefied version of magic.py by massimilianodelliubaldini")
print("Only difference being this version is it accepts the resolution as input (CLI)")
print("Original version can be found here: https://github.com/massimilianodelliubaldini/ac7-ultrawide")

print("IMPORTANT: If you used the script above in your main game folder before, please delete your ace7game.exe and use the unistall.bat file to revert the changes (and repair the game files on steam), so they dont conflict with nucleus ones")
time.sleep(5)
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        wait = input('ERROR: Please provide width and height as command line arguments. Press any key to close...')
        sys.exit(0)

    try:
        your_total_width = int(sys.argv[1])
        your_total_height = int(sys.argv[2])
        vertical_split = int(sys.argv[3])
    except ValueError:
        wait = input('ERROR: Width and height must be integers. Press any key to close...')
        sys.exit(0)
    ...
    print('Your instance size appears to be ' + str(your_total_width) + 'x' + str(your_total_height) + '.')
    ...
    while attempts >= 0:
        try:
            ...
            attempts -= 1
    try:       
        os.remove('Ace7Game.exe_' + timestamp)
    except OSError as e:
        print('WARNING: Unable to delete backup game exe.')
    ...
    if not os.path.exists('Mods/' + hudfix_filename):
        ...
        with open('Mods/' + hudfix_filename, 'r+') as hudfix_ini:
            ...
            hudfix_ini.close()
        if vertical_split == 1:
            hudfix_ini_data = hudfix_ini_data.replace(r'${0}\nadd ${register}, ${register}, l(-0.0000)', r'${0}\nmul ${register}, ${register}, l(2.0)')
        else:
            hudfix_ini_data = hudfix_ini_data.replace(r'${0}\nadd ${register}, ${register}, l(-0.0000)', r'${0}\nadd ${register}, ${register}, l(' + str(hud_shift_amount) + ')')
